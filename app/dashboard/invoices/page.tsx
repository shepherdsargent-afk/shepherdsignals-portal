          ...
          <div className="space-y-2">
            {invoices.map((inv: any) => (
              <div key={inv.id} className="flex items-center gap-4 p-3 rounded-lg hover:bg-white/5 transition-colors group">
                ...
                {inv.total_amount && (
                  <p className="text-white font-medium">${Number(inv.total_amount).toFixed(2)}</p>
                )}
                <span className={`text-xs px-2 py-1 rounded-full capitalize ${statusColor[inv.status] ?? ''}`}>
                  {inv.status}
                </span>
                {inv.file_url && (
                  <a href={inv.file_url} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-white text-xs shrink-0">
                    View
                  </a>
                )}
                <button
